...
EoF
	...
	OWNER = root
	GROUP = sys
	DFLAGS = -DSYSV -DPOSIX -DSOLARIS -DLOCKDIR=\"/var/spool/locks\" -DHASTZ $CM17AFLAG $EXT0FLAG $RFXSENFLAG $RFXMETFLAG $DMXFLAG $OREFLAG $KAKUFLAG $FLAGS_FLAG $TIMERS_FLAG $COUNTERS_FLAG
...
EoF
	...
	OWNER = root
	GROUP = sys
	DFLAGS = -DSYSV -DPOSIX -DSOLARIS  -DSYSBASEDIR=\"/opt/heyu/etc\" -DLOCKDIR=\"/var/spool/locks\" -DHASTZ $CM17AFLAG $EXT0FLAG $RFXSENFLAG $RFXMETFLAG $DMXFLAG $OREFLAG $KAKUFLAG
	CFLAGS = -g -O \$(DFLAGS) \$WALLFLAG
	LIBS = -lm -lc -lrt
EoF
	;;
...
EoF
	...
	CFLAGS = -g -O \$(DFLAGS) -Wall
	LIBS = -lm -lc
	DFLAGS= -DPOSIX -DNEEDGTIME -DFREEBSD -DLOCKDIR=\"/var/spool/lock\" -DSYSBASEDIR=\"/usr/local/etc/heyu\" -DSPOOLDIR=\"/var/tmp/heyu\" $CM17AFLAG $EXT0FLAG $RFXSENFLAG $RFXMETFLAG $DMXFLAG $OREFLAG $KAKUFLAG $FLAGS_FLAG $TIMERS_FLAG $COUNTERS_FLAG
EoF
	;;
    openbsd)
        cat >> Makefile <<-EoF
	...
	CFLAGS = -g -O \$(DFLAGS) -Wall
	LIBS = -lm -lc
	DFLAGS= -DPOSIX -DNEEDGTIME -DOPENBSD -DLOCKDIR=\"/var/spool/lock\" -DSYSBASEDIR=\"/etc/heyu\" -DSPOOLDIR=\"/var/tmp/heyu\" $CM17AFLAG $EXT0FLAG $RFXSENFLAG $RFXMETFLAG $DMXFLAG $OREFLAG $KAKUFLAG $FLAGS_FLAG $TIMERS_FLAG $COUNTERS_FLAG
EoF
       ;;
    netbsd)
	cat >> Makefile <<-EoF
